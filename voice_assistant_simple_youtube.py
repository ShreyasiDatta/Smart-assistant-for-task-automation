...
# Use googletrans instead of deep-translator for better compatibility
try:
...
# NEW: Simple YouTube search using requests (no external library needed)
try:
    import requests
...
# --- NEW: Simple YouTube Search Function ---
# --- NEW: Simple YouTube Search Function ---
def search_youtube_simple(query):
    """
    Simple YouTube search using requests - no external library needed.
    Returns the first video ID or None.
    """
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # This looks for video IDs in multiple formats that YouTube uses
            ...
            if matches:
                # re.findall with OR groups returns tuples, e.g., ('', 'VIDEO_ID') or ('VIDEO_ID', '')
                # We need to find the first non-empty ID.
                for match_tuple in matches:
                    # Get the first non-empty string from the tuple (either group 1 or group 2)
                    video_id = next((id for id in match_tuple if id), None) 
                    
                    if video_id:
                        print(f"First valid video ID: {video_id}")
                        return video_id  # Return first valid video ID
                
                print("No valid video IDs found in the matches")
                return None
            
            else:
                print("No video IDs found in response text")
...
def handle_youtube_command(command):
    ...
    if video_id:
        return {
            ...
            "video_id": video_id,
            "query": query,
            "video_title": query,  # We don't have the actual title, use query
...
<body class="bg-gray-100 font-sans p-4 sm:p-8">
    <div class="max-w-3xl mx-auto bg-surface shadow-xl container p-6 sm:p-10">
        ...
        <p class="text-center text-sm text-gray-600 mb-8">
            Try: 'read report.pdf', 'play cat video on youtube', or 'search for best books'.
        </p>
    ...
    <script>
        ...
        async function submitCommand() {
          alert("DEBUG: 1. 'submitCommand' function started."); // <-- NEW
          ...
          alert("DEBUG: 2. UI updated. Command is: " + command); // <-- NEW

          stopReading();
          
          try {
              alert("DEBUG: 3. Trying to 'fetch' from server..."); // <-- NEW
              ...
              alert("DEBUG: 4. Server responded."); // <-- NEW
              ...
              alert("DEBUG: 5. A CATCH ERROR happened: " + error.message); // <-- NEW
...
if __name__ == '__main__':
    ...
    print(f"NEW FEATURE: Say 'play [video name] on youtube' for autoplay!")
    
    print(f"\n--- Library Status ---")
    print(f"Requests Available: {REQUESTS_AVAILABLE}")
    print(f"Translator Available: {TRANSLATOR_AVAILABLE}")
    if REQUESTS_AVAILABLE:
        print(f"✓ YouTube autoplay enabled (using simple search)!")
    else:
        print(f"✗ YouTube autoplay disabled - install: pip install requests")
    print(f"---------------------\n")
